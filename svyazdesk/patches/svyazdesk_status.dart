// СвязьDesk: состояние тарифа в главном окне клиента.
//
...
class SvyazDeskStatusCard extends StatefulWidget {
  const SvyazDeskStatusCard({Key? key}) : super(key: key);

  @override
  State<SvyazDeskStatusCard> createState() => _SvyazDeskStatusCardState();
}

class _SvyazDeskStatusCardState extends State<SvyazDeskStatusCard> {
  SvyazDeskStatus? _status;
  Timer? _timer;
  ...
  Future<void> _load() async {
    try {
      ...
      final base = apiServer.isNotEmpty ? apiServer : kSvyazDeskSite;
      ...
      final cabinet = (data['cabinetUrl'] ?? data['siteUrl'] ?? kSvyazDeskSite).toString();
      final tariffs = (data['tariffUrl'] ?? kSvyazDeskSite).toString();

      final status = SvyazDeskStatus(
